...
module.exports = async (client, guildMember) => {
    ...
    .setDescription(`${guildMember} \`\`(${guildMember.user.username}#${guildMember.user.discriminator} - ${guildMember.user.id})\`\`, Sunucudan ayrıldı.\n Sunucudaki üye sayısı **${guildMember.guild.members.cache.filter(m => !m.user.bot).size}** oldu.`)
    .setColor("RED");
    guildMember.guild.channels.cache.get(config.welcome.channelId).send({ embeds: [embed] })
}